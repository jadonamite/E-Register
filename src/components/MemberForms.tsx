...
const Label = ({ children }: { children: React.ReactNode }) => (
  <label className="text-[10px] font-black text-zinc-400 uppercase ml-2 tracking-widest">{children}</label>
);

const GlassInput = (props: any) => (
  <Input 
    {...props} 
    className="h-12 px-5 rounded-2xl border-none bg-white/30 focus:bg-white/60 transition-all ring-0 focus-visible:ring-2 focus-visible:ring-white/50 text-sm" 
  />
);
  ...
  <div className="space-y-6">
    <div className="grid grid-cols-2 gap-4">
      <div className="col-span-2 space-y-1.5">
        ...
        <GlassInput placeholder="Enter member name..." />
      </div>
    ...
    <SubmitButton label="Add to Database" onClick={() => onSubmit({})} />
  </div>
);
...
const SubmitButton = ({ label, onClick }: { label: string, onClick: () => void }) => (
  <Button onClick={onClick} className="w-full h-16 rounded-3xl bg-black text-white hover:bg-zinc-900 text-lg font-black shadow-2xl transition-all active:scale-[0.98]">
    {label}
  </Button>
);